...
void BTree::generateDotFile(const std::string& filePath) const {
    ...
    if (svgResult != 0) {
        std::cout << "Generación fallida del SVG B_Tree. Asegurese de tener instalado Graphviz" << std::endl;
    } else {
        std::cout << "Generación exitosa del SVG B_Tree en: " << svgPath << std::endl;
    }
}